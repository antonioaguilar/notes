...
function printHelpAndExit {
  ...
  echo ''
  echo 'Example:'
  echo '  mov2gif -c 240:80 -o myimage.gif -x mymovie.mov'
  exit $1
}
...
echo 'Mov2Gif: Converting movie ...'
delay=$(bc -l <<< "100/$fps/$speed")
temp=$(mktemp /tmp/tempfile.XXXXXXXXX)

ffmpeg -loglevel info -i $filename $filter -r $fps -f image2pipe -vcodec ppm - >> $temp

echo 'Mov2Gif: Exporting gif ...'
